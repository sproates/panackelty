...
{
    while (value->len && value->limbs[value->len - 1] == 0) {
        value->len--;
    }
    if (!value->len) {
        value->sign = 0;
...
{
    if (length > SIZE_MAX / sizeof(uint32_t)) {
        return false;
    }
    ...
    if (length > value->len) {
        memset(next + value->len, 0, (length - value->len) * sizeof(uint32_t));
    }
    value->limbs = next;
    value->len = length;
    return true;
...
bool pn_big_add_small(PnBigInt *value, uint32_t addend)
{
    ...
    if (!value->sign) {
        pn_big_free(value);
        return pn_big_from_u64(value, addend);
    }
...
bool pn_big_from_bytes(PnBigInt *out, const uint8_t *bytes, size_t length)
{
    ...
    for (size_t i = 0; i < length; i++) {
        if (!pn_big_mul_small(out, 256) || !pn_big_add_small(out, bytes[i])) {
            pn_big_free(out);
            return false;
        }
    }
...
bool pn_big_from_digits(PnBigInt *out, const char *digits, size_t length, int sign)
{
    ...
    for (size_t i = 0; i < length; i++) {
        if (digits[i] < '0' || digits[i] > '9' || !pn_big_mul_small(out, 10) ||
            !pn_big_add_small(out, (uint32_t)(digits[i] - '0'))) {
            pn_big_free(out);
            return false;
        }
    }
...
char *pn_big_string(const PnBigInt *value)
{
    ...
    for (size_t i = value->len - 1; i-- > 0;) {
        at += (size_t)snprintf(text + at, cap - at, "%09u", value->limbs[i]);
    }
    return text;
}
...
{
    if (!a->sign) {
        return pn_big_copy(out, b);
    }
    if (!b->sign) {
        return pn_big_copy(out, a);
    }
    bool ok;
    if (a->sign == b->sign) {
        ok = abs_add(out, a, b);
...
bool pn_big_mul(PnBigInt *out, const PnBigInt *a, const PnBigInt *b)
{
    ...
    if (a->len > SIZE_MAX - b->len || !resize(out, a->len + b->len)) {
        return false;
    }
    for (size_t i = 0; i < a->len; i++) {
        uint64_t carry = 0;
        for (size_t j = 0; j < b->len; j++) {
            ...
            out->limbs[i + j] = (uint32_t)(current % BASE);
            carry = current / BASE;
        }
        size_t k = i + b->len;
        while (carry) {
            uint64_t current = out->limbs[k] + carry;
            out->limbs[k++] = (uint32_t)(current % BASE);
            carry = current / BASE;
        }
    }
...
static bool shift_base_add(PnBigInt *value, uint32_t limb)
{
    ...
    memmove(value->limbs + 1, value->limbs, old * sizeof(uint32_t));
    value->limbs[0] = limb;
    value->sign = 1;
    return true;
}
...
bool pn_big_divmod(PnBigInt *q, PnBigInt *r, const PnBigInt *a, const PnBigInt *b)
{
    ...
    aa.sign = aa.len ? 1 : 0;
    bb.sign = bb.len ? 1 : 0;
    if (abs_compare(&aa, &bb) < 0) {
        pn_big_free(&aa);
        pn_big_free(&bb);
        return pn_big_copy(r, a);
    }
    if (!resize(q, aa.len)) {
        pn_big_free(&aa);
        pn_big_free(&bb);
    ...
    for (size_t pos = aa.len; pos-- > 0;) {
        if (!shift_base_add(r, aa.limbs[pos])) {
            goto fail;
        }
...
{
    return value->sign == 1 && value->len == 1 && value->limbs[0] == 1;
}
...
bool pn_big_fits_size(const PnBigInt *value, size_t *out)
{
    ...
    size_t result = 0;
    for (size_t i = value->len; i-- > 0;) {
        if (result > (SIZE_MAX - value->limbs[i]) / BASE) {
            return false;
        }
        result = result * BASE + value->limbs[i];
    }
    *out = result;
    return true;
}
