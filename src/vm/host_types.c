...
{
    if (!strcmp(name, "path_current")) {
        return value_data(V_PATH, (const uint8_t *)".", 1);
    }
    if (!strcmp(name, "path_from_text") || !strcmp(name, "path_from_native")) {
        ...
        if (!a[0]->as.bytes.length) {
            return host_error("EmptyPath");
        }
        if (memchr(a[0]->as.bytes.data, 0, a[0]->as.bytes.length)) {
            return host_error("PathContainsNul");
        }
    ...
    if (!strncmp(name, "path_", 5)) {
        ...
        if (!strcmp(name, "path_to_text")) {
            if (!utf8(data, length)) {
                return host_error("PathNotUtf8");
            }
            return host_variant("Ok", value_data(V_STR, data, length));
        }
        if (!strcmp(name, "path_native_bytes")) {
            return value_data(V_BYTES, data, length);
        }
        if (!strcmp(name, "path_absolute")) {
            return value_bool(data[0] == '/');
        }
        ...
        if (!strcmp(name, "path_append")) {
            ...
            if (a[1]->as.bytes.data[0] == '/') {
                return host_error("AbsolutePathAppend");
            }
            Buffer out = {0};
            ...
            Value *result =
                ok ? host_variant("Ok", value_data(V_PATH, (const uint8_t *)out.data, out.length))
                   : NULL;
            free(out.data);
            return result;
        }
        size_t root = 0, end = length;
        while (root < length && data[root] == '/') {
            root++;
        }
        while (end > root && data[end - 1] == '/') {
            end--;
        }
        size_t start = end;
        while (start > root && data[start - 1] != '/') {
            start--;
        }
        if (!strcmp(name, "path_filename")) {
            if (end == root) {
                return named_value(V_VARIANT, "None", NULL, NULL, 0);
            }
            return host_variant("Some", value_data(V_PATH, data + start, end - start));
        }
        if (start <= root) {
            return root ? value_data(V_PATH, data, root)
                        : value_data(V_PATH, (const uint8_t *)".", 1);
        }
    ...
    if (!strcmp(name, "duration_from_seconds")) {
        ...
        Value *result = !ok                     ? NULL
                        : !pn_big_is_zero(&rem) ? host_error("FractionalNanosecond")
    ...
    if (!strcmp(name, "instant_now")) {
        ...
        if (clock_gettime(CLOCK_MONOTONIC, &now)) {
            return host_error("ClockUnavailable");
        }
        PnBigInt ticks = {0};
        bool ok = pn_big_from_u64(&ticks, (uint64_t)now.tv_sec) &&
                  pn_big_mul_small(&ticks, 1000000000) &&
                  pn_big_add_small(&ticks, (uint32_t)now.tv_nsec);
    ...
    if (!strcmp(name, "instant_before")) {
        return value_bool(pn_big_compare(&a[0]->as.integer, &a[1]->as.integer) < 0);
    }
    PnBigInt ticks = {0};
