...
| Panackelty-hosted fixture orchestration | `runner/main.panack`; `functional/cases/runner_smoke`; `functional/cases/cli_check_disasm`; `functional/cases/cli_commands`; `unit/compiler/test_panackelty_runner.py`; existing Python functional harness | **Partial** — twenty-two success fixtures and twenty example programs run source, compile, and bytecode with byte-exact streams/status; forty-one failure fixtures check exact normalized diagnostics for check/compile and absent artifacts. Source and bytecode checks, matching disassembly, malformed bytecode and legacy extension rejection, bare-path execution, default compile output, arguments, stderr/exit status, help and version, paired example discovery, explicit artifact and workspace cleanup, deterministic reports, injected output/diagnostic mismatches, missing fixture expectations, nonempty-workspace recovery, isolated inherited environment for `stdlib`, and checkout-bound compiler `source.path` are covered. The Python harness remains required for other host errors and remaining environment and CLI tests. |
